...
function filterSearchResults(results, compound) {
  return (results || []).filter(r => {
    if (!compoundAppears(r, compound)) return false;
...
function renderResults(results) {
  ...
  if (!lastSearchResults.length) {
    container.innerHTML = '<div class="text-muted">No results found with the compound explicitly present in the returned title/snippet/URL.</div>';
    return;
  }
...
async function runSearch(evt) {
  ...
  try {
    ...
    renderResults(filterSearchResults(data.results || [], compound));
  } catch (err) {
...
async function createSummary() {
  ...
  if (!reportable.length) { alert('Select at least one actual article/public record. Jump-off search links are excluded from the report.'); return; }
  const payload = {
    compound,
    keywords: getKeywords(),
