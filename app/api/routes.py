...
async def search(payload: SearchRequest):
    ...
    for source in requested:
        ...
        if not connector:
            continue
        try:
            source_results = await connector.search(
                payload.compound,
                payload.keywords,
...
def create_summary(payload: SummaryRequest):
    ...
    intro = [
        ...
        "",
        f"**Query focus:** {payload.compound}"
        + (f"; keywords: {', '.join(payload.keywords)}" if payload.keywords else ""),
        "",
    ...
    if contradictions:
        sections.append(
            "Some retrieved snippets may not align fully with one another or may indicate negative or limited findings. These should be compared in the full text before concluding consistency."
        )
        sections += [f"- Potentially divergent or limited wording retrieved from: {c}" for c in contradictions]
    else:
        sections.append(
            "No explicit contradiction was identifiable from the retrieved snippets alone. This is not confirmation of agreement; it only reflects the available text retrieved in-app."
        )
    ...
    if payload.user_notes:
        ...
        sections.append(
            "Add reviewer interpretation, extraction decisions, and any manual findings from full-text review here."
        )

    sections += ["", "## References", ""] + refs

    report_text = "\n".join(sections)
    return SummaryResponse(report_markdown=report_text)
